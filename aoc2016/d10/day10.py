...
raw_puzzle_input = '''bot 88 gives low to bot 51 and high to bot 42
...
class Solver(object):
    def __init__(self, puzzle_input, compare_requirement):
        self.puzzle_input = puzzle_input
        self.bots = []
        self.compare_requirement = compare_requirement
        self.output = [0, 0, 0]

    def initiate_bots(self):
        bots = re.findall(r'bot (\d+)', self.puzzle_input)
        bots_count = len(set(bots))
        self.bots = [[] for _ in range(bots_count)]

    def give_initial_chips(self):
        chips = re.findall(r'value (?P<value>\d+) goes to bot (?P<bot_id>\d+)', self.puzzle_input)
        for value, bot_id in chips:
            self.bots[int(bot_id)].append(int(value))
    ...
    def go(self):
        ...
        part1_answer = -1
        part2_answer = 0
        while not (part1_answer >= 0 and part2_answer):
            active_bot_id, chips = list(filter(lambda x: len(x[1]) == 2, enumerate(self.bots)))[0]
            if chips[0] in self.compare_requirement and chips[1] in self.compare_requirement:
                part1_answer = active_bot_id
            command = re.search(r'bot {} gives low to (?P<to_low>bot|output) (?P<low_id>\d+) and high'
                                r' to (?P<to_high>bot|output) (?P<high_id>\d+)'.format(active_bot_id),
                                self.puzzle_input).groupdict()
            self.bots[active_bot_id] = []

            if command['to_low'] == 'bot':
                self.bots[int(command['low_id'])].append(min(chips))
            if command['to_high'] == 'bot':
                self.bots[int(command['high_id'])].append(max(chips))
            if command['to_low'] == 'output' and int(command['low_id']) in range(3) and not self.output[int(command['low_id'])]:
                self.output[int(command['low_id'])] = min(chips)
            if command['to_high'] == 'output' and int(command['high_id']) in range(3) and not self.output[int(command['high_id'])]:
                self.output[int(command['high_id'])] = max(chips)
            part2_answer = self.output[0] * self.output[1] * self.output[2]
        return part1_answer, part2_answer


if __name__ == '__main__':
    s = Solver(raw_puzzle_input, (61, 17,))
    print(s.go())
