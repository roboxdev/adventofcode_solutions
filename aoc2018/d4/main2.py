    ...
    def go(self):
        ...
        for entry in sorted_log:
            if entry.guard_id:
                ...
                multiplier = (entry.day - sleeps_since.day) + 1
                sleeping_minutes[current_guard] += list(range(sleeps_since.minute, entry.minute)) * multiplier
                sleeps_since = None
        counters = {k: Counter(v).most_common(1) for k, v in sleeping_minutes.items()}
        winner_guard = max(counters.items(), key=lambda x: x[1][0][1] if x[1] else 0)
        return winner_guard[0] * winner_guard[1][0][0]
