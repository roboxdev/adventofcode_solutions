...
raw_puzzle_input = '''fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
...
class Solver(object):
    expr = r'^(?P<name>.+)-(?P<sector_id>[\d]+)\[(?P<checksum>[\w]{5})\]'
    real_rooms = []
    ...
    def go(self):
        sector_ids_sum = 0
        for room in self.puzzle_input:
            match = re.match(self.expr, room)
            name, sector_id, checksum = match.group('name'), int(match.group('sector_id')), match.group('checksum')
            name_without_hyphens = name.replace('-', '')
            counts = collections.Counter(name_without_hyphens).items()
            s = sorted(counts, key=lambda x: (-x[1], x[0]))
            a = ''.join([d[0] for d in s][:5])
            if a == checksum:
                sector_ids_sum += sector_id
                self.real_rooms.append((name, sector_id, checksum))
        return sector_ids_sum

    def go2(self):
        result = 0
        for name, sector_id, checksum in self.real_rooms:
            alphabet = string.ascii_lowercase
            shift = sector_id % len(alphabet)
            shifted_alphabet = alphabet[shift:] + alphabet[:shift]
            table = str.maketrans(alphabet, shifted_alphabet)
            real_name = name.translate(table)
            print('{} ({})'.format(real_name, name))
