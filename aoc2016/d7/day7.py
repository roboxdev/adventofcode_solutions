...
raw_puzzle_input = '''rhamaeovmbheijj[hkwbkqzlcscwjkyjulk]ajsxfuemamuqcjccbc
...
class Solver(object):
    ...
    def abba_checker(self, substring):
        if substring[0] == substring[3] and substring[1] == substring[2] and substring[0] != substring[1]:
            return True
        else:
            return False

    def sequencer(self, string):
        for i in range(len(string) - 3):
            if self.abba_checker(string[i:i + 4]):
                return True

    def sequencer2(self, string):
        return [string[i:i + 3] for i in range(len(string) - 2)]

    def go(self):
        for ip in self.puzzle_input:
            hypernet_re = r'\[(.+?)\]'
            hypernets = re.findall(hypernet_re, ip)
            supernet = ip
            for hypernet in hypernets:
                supernet = supernet.replace(hypernet, '')
            supernet = supernet.split('[]')
            supernet_bool = any(map(self.sequencer, supernet))
            hypernets_bool = not any(map(self.sequencer, hypernets))
            self.counter += 1 if supernet_bool and hypernets_bool else 0
        return self.counter
    ...
    def go2(self):
        for ip in self.puzzle_input:
            ...
            for hypernet in hypernets:
                supernet = supernet.replace(hypernet, '')
            supernet = supernet.split('[]')
            supernet_chunks = map(self.sequencer2, supernet)
            supernet_chunks = reduce(lambda x, y: x + y, supernet_chunks)
            supernet_chunks_filtered = filter(lambda x: x[0] == x[2] != x[1], supernet_chunks)
            supernet_chunks_babified = map(lambda x: x[1]+x[0]+x[1], supernet_chunks_filtered)
            chunks_producted = product(supernet_chunks_babified, hypernets)
            self.counter += 1 if any(map(lambda x: x[0] in x[1], chunks_producted)) else 0
        return self.counter
